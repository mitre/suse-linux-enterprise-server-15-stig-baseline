...
This InSpec Profile was created to facilitate testing and auditing of `SUSE Linux Enterprise Server 15`
...
The SUSE Linux Enterprise Server 15 STIG Profile uses the [InSpec](https://github.com/inspec/inspec)
...
Requirements associated with the SUSE Linux Enterprise Server 15 STIG are derived from the
...
The SUSE Linux Enterprise Server 15 STIG profile checks were developed to provide technical implementation
...
```yaml
  ...
  # used by the SUSE Linux Enterprise Server 15 STIG profile.
...
```bash
# Using `ssh` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/suse-linux-enterprise-server-15-stig-baseline/archive/main.tar.gz --input-file=<your_inputs_file.yml> -t ssh://<hostname>:<port> --sudo --reporter=cli json:<your_results_file.json>

# Using `winrm` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/suse-linux-enterprise-server-15-stig-baseline/archive/master.tar.gz --target winrm://<hostip> --user '<admin-account>' --password=<password> --input-file=<path_to_your_inputs_file/name_of_your_inputs_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/suse-linux-enterprise-server-15-stig-baseline.git
bundle exec [inspec or cinc-auditor] archive suse-linux-enterprise-server-15-stig-baseline
...
```bash
cd suse-linux-enterprise-server-15-stig-baseline
git pull
cd ..
bundle exec [inspec or cinc-auditor] archive suse-linux-enterprise-server-15-stig-baseline --overwrite
